...
export default async function getFavoritedPlaces(
  ...
  let query = MC.UserFavoritedPlaces.find({ userId: req.params.id })
    .skip(skip)
    .limit(limit)
    .populate({
      path: "place",
  ...
  const favoritedPlaces = await query.exec();

  return favoritedPlaces.map((favoritedPlace) =>
