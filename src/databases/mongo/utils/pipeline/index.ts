...
// Import types
import type { LookupStage } from "../../index.types";

export class PipelineUtil extends Base {
  /**
  ...
  getLimitnSkipStage(limit: number, skip: number) {
    if (limit === undefined || limit === null)
      throw new Error(
        "PipelineUtil - Fn: getLimitnSkipStage - '[limit] is required'"
      );
    if (skip === undefined || skip === null)
      throw new Error(
        "PipelineUtil - Fn: getLimitnSkipStage - '[skip] is required'"
      );
  ...
  or(...matchQueriesArray: Array<any>) {
    return {
      $or: matchQueriesArray.filter(function (matchQuery) {
        return (
          typeof matchQuery === "object" && Object.keys(matchQuery).length > 0
        );
      }),
    };
  }
  ...
  and(...matchQueriesArray: Array<any>) {
    return {
      $and: matchQueriesArray.filter(function (matchQuery) {
        return (
          typeof matchQuery === "object" && Object.keys(matchQuery).length > 0
        );
      }),
   ...
   */
  getProjectStage(
    shownfields?: Array<string> | string,
    hiddenfields?: Array<string> | string,
    replace?: Array<[string, any]>
  ) {
    let fields: { [K: string]: any } = {};

    if (!this.utils.boolean.isEmpty(shownfields)) {
    ...
    if (!this.utils.boolean.isEmpty(hiddenfields)) {
    ...
    if (!this.utils.boolean.isEmpty(replace))
      for (const newProjection of replace)
        if (!this.utils.boolean.isEmpty(fields[newProjection[0]]))
          fields[newProjection[0]] = newProjection[1];
  ...
  getLookupStage(
    ...
    if (!this.utils.boolean.isEmpty(pipeline)) {
